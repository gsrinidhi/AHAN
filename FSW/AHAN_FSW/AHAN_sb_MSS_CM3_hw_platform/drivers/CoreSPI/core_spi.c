 ...
 * SPI slave operations with the CoreSPI version 4.2
 * with CoreSPI version 3.0
 *
 * SVN $Revision: 9667 $
...
)
{
    ...
    if( NULL_INSTANCE != this_spi )
    {
        ...
        HAL_set_8bit_reg( this_spi->base_addr, CTRL1, 0x60 );
        stat = HAL_get_8bit_reg( this_spi->base_addr, CTRL1);
    }
}
